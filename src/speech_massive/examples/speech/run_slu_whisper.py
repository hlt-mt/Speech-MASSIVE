...
from speech_massive.examples.scripts.massive_eval import MassiveEval
from speech_massive.examples.scripts import s3prl_slot_eval
from speech_massive.examples.speech.trainer_seq2seq_whisper_slu import CustomSeq2SeqTrainer
...
class ModelArguments:
    ...
    model_revision: str = field(
        default="main",
        metadata={
            "help": "The specific model version to use (can be a branch name, tag name or commit "
                    "id)."})
    token: str = field(
        default=None,
        metadata={
            "help": "The token to use as HTTP bearer authorization for remote files. If not "
    ...
    trust_remote_code: bool = field(
        default=False,
        metadata={
            "help": "Whether or not to allow for custom models defined on the Hub in their own "
                    "modeling files. This option should only be set to `True` for repositories "
                    "you trust and in which you have read the code, as it will execute code "
    ...
    forced_decoder_ids: List[List[int]] = field(
        default=None,
        metadata={
            "help": "A list of pairs of integers which indicates a mapping from generation "
                    "indices to token indices that will be forced before sampling. For example, "
                    "[[0, 123]] means the first generated token will always be a token of index "
                    "123."})
    ...
    apply_spec_augment: bool = field(
        ...
        metadata={
            "help": "Whether to apply *SpecAugment* data augmentation to the input features. "
                    "This is currently only relevant for Wav2Vec2, HuBERT, WavLM and Whisper "
                    "models."})
...
class DataTrainingArguments:
    ...
    task: str = field(
        metadata={
            "help": "Task, either `transcribe` for speech recognition or `translate` for speech "
                    "translation."})
    dataset_name: str = field(
        default=None,
        metadata={
            "help": "The name of the dataset to use for train (via the datasets library)."})
    ...
    is_few_shot: bool = field(
        default=False,
        metadata={
            "help": "Whether to train few-shot or not. Default is False."})
    eval_dataset_name: str = field(
        default=None,
        metadata={
            "help": "The name of the dataset to use for evaluation (via the datasets library)."})
    eval_dataset_config_name: Optional[str] = field(
        default=None,
        metadata={
            "help": "The configuration name of the EVAL dataset to use (via the datasets "
                    "library)."})
    ...
    test_dataset_config_name: Optional[str] = field(
        default=None,
        metadata={
            "help": "The configuration name of the TEST dataset to use (via the datasets "
                    "library)."})
    slurp_dataset_path: str = field(
        default=None,
        metadata={
            "help": "Give slurp dataset path to use slurp dataset."})
    slurp_dataset_config_name: Optional[str] = field(
        default="slurp_real",
        metadata={
            "help": "The configuration name of the slurp dataset."})
    train_slurp: bool = field(
        default=False,
        metadata={
            "help": "Whether or not to train on SLURP train set."})
    eval_slurp: bool = field(
        default=False,
        metadata={
            "help": "Whether or not to eval on SLURP dev set."})
    italic_dataset_path: str = field(
        default=None,
        metadata={
            "help": "Give italic dataset path to use italic dataset."})
    italic_dataset_config_name: Optional[str] = field(
        default="italic-train",
        metadata={
            "help": "The configuration name of the italic dataset."})
    train_italic: bool = field(
        default=False,
        metadata={
            "help": "Whether or not to train on italic train set."})
    eval_italic: bool = field(
        default=False,
        metadata={
            "help": "Whether or not to eval on italic dev set."})
    ...
    audio_column_name: str = field(
        default="audio",
        metadata={
            "help": "The name of the dataset column containing the audio data. Defaults to "
                    "'audio'."})
    ...
    max_duration_in_seconds: float = field(
        default=30.0,
        metadata={
            "help": "Truncate audio files that are longer than `max_duration_in_seconds` seconds "
                    "to 'max_duration_in_seconds`."})
    ...
    preprocessing_only: bool = field(
        default=False,
        metadata={
            "help": "Whether to only do data preprocessing and skip training. This is especially "
                    "useful when data preprocessing errors out in distributed training due to "
                    "timeout. In this case, one should run the preprocessing in a non-distributed "
                    "setup with `preprocessing_only=True` so that the cached datasets can "
    ...
    replace_task_token: bool = field(
        ...
        metadata={
            ...
            "help": "Whether to replace <transcribe> or <translate> tag with <startoflm> tag."})
    add_slu_tag: bool = field(
        default=False,
        metadata={
            "help": "Whether to use <startoflm> right after <transcribe> or <translate> tag to "
                    "indicate the SLU task."})
    add_separator: str = field(
        default=None,
        metadata={
            "help": "If no separator is given, it will be without the separator. Make sure add "
                    "tokenizer split separator(Ġ)."})
    tokens_to_remove_from_suppress: Optional[List[str]] = field(
        default=None,
        metadata={
            "help": "If true, remove separator from the suppressed token if it exists."})
    ...
    target_format_content: str = field(
        ...
        metadata={
            "help": "Options: transcript_slots_intent, slots_intent"})
    target_format_structure: str = field(
        default=None,
        metadata={
            "help": "Options: none, natural, json."})
...
class CustomArguments:
    ...
    do_early_stopping: bool = field(
        default=True,
        metadata={
            "help": "Whether to do early stop or not. Default True."})
    early_stopping_patience: Optional[int] = field(
        default=10,
        metadata={
            "help": "Number of evaluation calls with no improvement after which training will be "
                    "stopped."})
...
class DataCollatorSpeechSeq2SeqWithPadding:
    ...
    def __call__(
        ...
        # if bos token is appended in previous tokenization step, cut bos token here as it's append
        # later anyways
...
def main():
    ...
    # See all possible arguments in src/transformers/training_args.py or by passing the --help flag
    # to this script. We now keep distinct sets of args, for a cleaner separation of concerns.
    ...
    if sys.argv[1].endswith(".yaml"):
        model_args, data_args, training_args, custom_args = (
            parser.parse_yaml_file(yaml_file=os.path.abspath(sys.argv[1])))
    ...
    if model_args.use_auth_token is not None:
        warnings.warn(
            "The `use_auth_token` argument is deprecated and will be removed in v4.34. Please use "
            "`token` instead.", FutureWarning)
        if model_args.token is not None:
            raise ValueError(
                "`token` and `use_auth_token` are both specified. Please set only the argument "
                "`token`.")
    ...
    logger.warning(
        ...
        f"{training_args.n_gpu}, distributed training: "
        f"{training_args.parallel_mode.value == 'distributed'}, 16-bits training: "
    ...
    if not data_args.dataset_name and not data_args.slurp_dataset_path and not \
            data_args.italic_dataset_path and not data_args.test_dataset_name:
        raise ValueError(
            "At least one dataset has to be provided. Either specify dataset_name or use "
            "use_slurp_dataset. Or test_data_name should be given.")
    ...
    if training_args.do_train:
        ...
        if data_args.slurp_dataset_path and data_args.train_slurp:
            logger.info("Slurp data set is being loaded for Training.")
        ...
        if data_args.italic_dataset_path and data_args.train_italic:
            logger.info("ITALIC data set is being loaded for Training.")
        ...
        if data_args.dataset_name:
            logger.info("Speech-MASSIVE data set is being loaded for Training.")
            ...
            for dataset_config in dataset_config_list:
                # avoid duplicated few-shot dataset for the main language
                few_shot_lang_list.remove(dataset_config)
                logger.info(f"Speech-MASSIVE {dataset_config} is being loaded for training.")
                massive_train_dataset_list.append(
                    load_dataset(
            ...
            if data_args.is_few_shot:
                for few_shot_lang in few_shot_lang_list:
                    logger.info(
                        f"Speech-MASSIVE {few_shot_lang} few-shot split is being loaded for "
                        "training.")
                    massive_train_dataset_list.append(
                        load_dataset(
                            data_args.dataset_name,
        ...
        if len(raw_datasets_list) == 0:
            raise ValueError("Something wrong with the loaded datasets.")
    ...
    if training_args.do_eval:
        ...
        if data_args.slurp_dataset_path and data_args.eval_slurp:
            logger.info("Slurp data set is being loaded for Evaluation.")
        ...
        if data_args.italic_dataset_path and data_args.eval_italic:
            logger.info("ITALIC data set is being loaded for Evaluation.")
        ...
        if data_args.eval_dataset_name:
            ...
            logger.info("Speech-MASSIVE data set is being loaded for Evaluation.")
        ...
        if slurp_eval_dataset and massive_eval_dataset and italic_eval_dataset:
            logger.info("Interleaving SLURP, ITALIC and Speech-MASSIVE dataset for evaluation.")
            massive_eval_dataset = massive_eval_dataset.remove_columns([
                "worker_id",
                "slot_method",
                "judgments",
                "bad_transcript_reported",
                "recorder_id",
                "validator_id"])
            ...
            if data_args.max_eval_samples:
                ...
                num_eval_samples_per_lang = (
                    data_args.max_eval_samples // num_eval_langs // training_args.eval_batch_size)
            else:
                num_eval_samples_per_lang = (
                    min_single_dataset_len // num_eval_langs // training_args.eval_batch_size)

            if num_eval_samples_per_lang == 0:
                raise ValueError("num_eval_samples_per_lang cannot be 0.")
            ...
            for eval_dataset_split in massive_eval_dataset.keys():
                rand_eval_idx_list = random.choices(
                    range(0, min_single_dataset_len), k=num_eval_samples_per_lang)
                selected_dataset_list.append(massive_eval_dataset[eval_dataset_split].select(
                        rand_eval_idx_list))

            rand_eval_idx_list = random.choices(
                range(0, len(slurp_eval_dataset)),
                k=num_eval_samples_per_lang)
            selected_dataset_list.append(slurp_eval_dataset.select(rand_eval_idx_list))

            rand_eval_idx_list = random.choices(
                range(0, len(italic_eval_dataset)),
                k=num_eval_samples_per_lang)
            ...
            logger.info(
                "Interleaving Speech-MASSIVE dataset's different languages for evaluation.")
            logger.info(f"Num eval langs : {num_eval_langs}")
            logger.info(f"Eval batch size : {training_args.eval_batch_size}")
            ...
            if data_args.max_eval_samples:
                logger.info(f"Num max eval samples : {data_args.max_eval_samples}")
                num_eval_samples_per_lang = data_args.max_eval_samples // num_eval_langs // \
                    training_args.eval_batch_size
            else:
                # few samples will be discarded as it will be the remainders
                num_eval_samples_per_lang = dataset_len // num_eval_langs // \
                    training_args.eval_batch_size
            ...
            for eval_dataset_split in massive_eval_dataset.keys():
                ...
                rand_eval_idx_list = random.choices(
                    range(0, dataset_len),
                    k=num_eval_samples_per_lang)
                selected_dataset_list.append(
                    massive_eval_dataset[eval_dataset_split].select(rand_eval_idx_list))
            eval_dataset = concatenate_datasets(selected_dataset_list)
        elif slurp_eval_dataset:
            logger.info("Using only SLURP dataset for evaluation.")
            logger.info(f"Num eval langs : {num_eval_langs}")
            logger.info(f"Eval batch size : {training_args.eval_batch_size}")
            ...
            logger.info("Using only ITALIC dataset for evaluation.")
            logger.info(f"Num eval langs : {num_eval_langs}")
            logger.info(f"Eval batch size : {training_args.eval_batch_size}")
    ...
    if training_args.do_predict:
        if data_args.test_dataset_name:
            logger.info(
                "Speech-MASSIVE data set is being loaded for Prediciton.")
    ...
    if data_args.audio_column_name not in next(iter(raw_datasets.values())).column_names:
        if training_args.do_predict and data_args.audio_column_name not in \
                next(iter(raw_datasets["test"].values())).column_names:
            raise ValueError(
                f"--audio_column_name '{data_args.audio_column_name}' not found in dataset "
                f"'{data_args.dataset_name}'. Make sure to set `--audio_column_name` to the "
                "correct audio column - one of "
                f"{', '.join(next(iter(raw_datasets.values())).column_names)}.")

    if data_args.text_column_name not in next(iter(raw_datasets.values())).column_names:
        if training_args.do_predict and data_args.text_column_name not in \
                next(iter(raw_datasets["test"].values())).column_names:
            raise ValueError(
                f"--audio_column_name '{data_args.audio_column_name}' not found in dataset "
                f"'{data_args.dataset_name}'. Make sure to set `--audio_column_name` to the "
    ...
    logger.warning("Setting model.config.forced_decoder_ids = None")
    logger.warning("This is to disable backward comaptibility in modeling_whisper.py")
    ...
    if data_args.tokens_to_remove_from_suppress:
        ...
        for token_to_release in token_list_to_release:
            ...
            if token_idx in model.generation_config.suppress_tokens:
                logger.warning(
                    f"Releasing [{token_to_release}] - [idx:{token_idx}] from suppressed tokens.")
                model.generation_config.suppress_tokens.remove(token_idx)
            else:
                logger.warning(
                    f"[{token_to_release}] - [idx:{token_idx}] is not in the suppressed tokens.")

    logger.warning(
        f"Updated models config - suppress_tokens : {model.generation_config.suppress_tokens}")
    ...
    # if SpecAugment is used for whisper models,
    # return attention_mask to guide the mask along time axis
    ...
    def prepare_dataset(
            ...
            add_slu_tag,
            replace_task_token,
            target_format_content,
            target_format_structure,
            separator):
        # process audio
        ...
        # ...
        # '<|notimestamps|>', 'O', 'ther', 'ĠOther', 'ĠOther',
        # 'Ġmusic', '_', 'qu', 'ery', '<|endoftext|>']
        ...
        if target_format_content == "transcript_slots_intent":
            if separator:
                ...
                if target_format_structure == "natural":
                    formatted_labels = f"Transcript: {transcript} {separator} Slots: {slots} " \
                                       f"{separator} Intent: {intent}"
            else:
                if target_format_structure is None:
        ...
        if replace_task_token:
            ...
            try:
                index_to_replace = batch["labels"].index(
            ...
            if not index_to_replace:
                raise ValueError(
                    f"Could not locate the task token in the labels {batch['labels']}.")
            batch["labels"][index_to_replace] = \
                tokenizer.convert_tokens_to_ids("<|startoflm|>")

        if add_slu_tag:
            if replace_task_token:
                raise ValueError(
                    "add_slu_tag cannot be done when replace_task_token is True. Make sure use "
                    "only one of them")
            try:
                index_to_insert_tag = batch["labels"].index(
                    tokenizer.convert_tokens_to_ids("<|transcribe|>")) + 1
            except ValueError:
                index_to_insert_tag = batch["labels"].index(
                    tokenizer.convert_tokens_to_ids("<|translate|>")) + 1
            batch["labels"].insert(
                index_to_insert_tag, tokenizer.convert_tokens_to_ids("<|startoflm|>"))
    ...
    with training_args.main_process_first(desc="dataset map pre-processing"):
        if training_args.do_predict:
            ...
            lang_list = [
                "_".join(test_set_name.split("_")[1:])
                for test_set_name in list(predict_dataset.keys())]
    ...
    def compute_metrics(pred):
        ...
        def _parse_prediction(target_format_content, separator, pred_list):
            ...
            for pred in pred_list:
                if target_format_content == "transcript_slots_intent":
                    ...
                    # this part will be used to evaluate the tool of NLU
        ...
        pred_parse_result_list = _parse_prediction(
            data_args.target_format_content,
            data_args.add_separator,
            pred_str)
        ...
        slot_type_f1 = s3prl_slot_eval.slot_type_f1(
            slots_pred_list,
            transcript_pred_list,
            slots_label_list,
            transcript_label_list)

        slot_value_cer = s3prl_slot_eval.slot_value_cer(
            slots_pred_list,
            transcript_pred_list,
            slots_label_list,
            transcript_label_list)
    ...
    if custom_args.do_early_stopping:
        ...
        trainer.add_callback(EarlyStoppingCallback(custom_args.early_stopping_patience))
            ...
            if trainer.is_world_process_zero():
                if training_args.predict_with_generate:
                    ...
                    output_label_file = os.path.join(
                        training_args.output_dir, f"{name}_labels.txt")
                    ...
                    annot_utt_w_idx = [
                        f"{sample_id}\t: {annot_utt.strip()}"
                        for annot_utt, sample_id in zip(annot_utt_list, sample_id_list)]
                    output_source_file = os.path.join(
                        training_args.output_dir, f"{name}_sources.txt")
                    with open(output_source_file, "w", encoding="utf-8") as writer:
                        writer.write("\n".join(annot_utt_w_idx))

    # 14. Write Training Stats
    kwargs = {
        "finetuned_from": model_args.model_name_or_path, "tasks": "automatic-speech-recognition"}
