    ...
    def prediction_step(
        ...
        **gen_kwargs,
        ...
        try:
            ...
            raise ValueError('<|notimestamps|> token does not exist in the tokenizer. Please check the tokenizer.')
        ...
        logger.warning(f'Models forced_decoder_ids : {self.model.generation_config.forced_decoder_ids}')
        if generation_inputs['labels'].shape[0] > 1: # bsz 1 for eval case
            logger.warning(
                f'Eval batch size {generation_inputs["labels"].shape[0]} is larger than 1\n'
                f'Below forced_token_ids will be FORCEFULLY applied to all the samples in this batch\n'
            )
        else:
            logger.warning(
                f'We are FORCING new forced_decoder_ids to support multilingual.\n'
                f'overall forced_decoder_ids : {forced_decoder_ids}\n'
                f'bos token will be automatically preprended in the modeling code.\n'
            )
        ...
        logger.warning(f'Reverting back to orginal models forced_decoder_ids')
